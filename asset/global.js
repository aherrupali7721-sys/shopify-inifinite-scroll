  ...
  if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 200) {
    ...
    fetch(window.location.pathname + "?page=" + nextPage)
      ...
      .then(data => {
        ...
        const products = html.querySelectorAll("#product-grid .grid__item");
...
function moveFeaturedToTop(){
  ...
  const featured = items.filter(p => p.dataset.featured === "true");
  const normal = items.filter(p => p.dataset.featured !== "true");
 
  grid.innerHTML = "";
 
  featured.forEach(p => grid.appendChild(p));
  normal.forEach(p => grid.appendChild(p));
...
function reorderFeatured() {
  ...
  const featured = items.filter(item => item.dataset.featured === "true");
  const normal = items.filter(item => item.dataset.featured !== "true");
 
  const topFeatured = featured.slice(0,15);
 
  const ordered = [...topFeatured, ...normal];
  ...
  if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 200) {
    ...
    fetch(window.location.pathname + "?page=" + nextPage)
      ...
      .then(data => {
        ...
        const newProducts = html.querySelectorAll(".product-card");
        const grid = document.querySelector("#product-grid");
 
        newProducts.forEach(p => {
          if(p.dataset.featured !== "true"){
            grid.appendChild(p);
          }
          moveFeaturedToTop();
